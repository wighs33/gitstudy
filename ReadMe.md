...
# 10. main game state 에 등장하는 game object
- class 구성정보
loading state가 끝나면 main_state로 변경
monster클래스에 몬스터정보, Unit클래스에 유닛정보가 들어가 있고
몬스터와 유닛이 화면에 나타난다.

# 11. 어려웠던/어려운 부분
- 다운받은 이미지의 배경이 투명이 아니여서 투명으로 만드는 방법을 겨우 찾았습니다.
- 구현이 막힌부분은 유닛을 마우스 클릭으로 선택해서 선택한 유닛만 이동시켜야 하는데 제대로 코딩했다고 생각했지만 계속 오류가 났습니다. 
